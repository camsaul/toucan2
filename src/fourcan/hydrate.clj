...
(m/defmulti automagic-hydration-key-model
  {:arglists '([k])}
  identity
  :default ::default
  :hierarchy #'hierarchy/hierarchy)

(m/defmethod automagic-hydration-key-model ::default
  [_]
  nil)
...
(m/defmethod hydrate-with-strategy ::automagic-batched
  ...
  (let [model       (automagic-hydration-key-model dest-key)
        ...
        ids         (set (for [result results
                               ...
                               :when  k]
                           k))
        primary-key (compile/primary-key model)
        objs        (if (seq ids)
...
(defn- batched-hydrate-dispatch-value [[first-result] k]
  (let [result-dispatch-val (types/model first-result)]
...
(defn- simple-hydrate-dispatch-value [result k]
  (let [result-dispatch-val (types/model result)]
...
(m/defmethod hydrate-with-strategy ::multimethod-simple
  ...
  ;; TODO - explain this
  (for [[first-result :as chunk] (partition-by types/model results)
...
(defn- hydrate-key
  ...
  (if-let [strategy (hydration-strategy results k)]
    (do
      (when debug/*debug*
        (printf "Hydrating %s with strategy %s\n" k strategy))
