...
(defn instance?
  "True if `x` is a Toucan2 instance, i.e. a `toucan2.instance_2.Instance` or some other class that satisfies the correct
  interfaces.
...
(defn instance
  (^toucan2.instance_2.Instance [model]
   (instance model (empty-map model)))

  (^toucan2.instance_2.Instance [model m]
   (let [m* (into (empty-map model) m)]
     (->Instance model m* m* (meta m))))

  (^toucan2.instance_2.Instance [model k v & more]
