...
(deftest override-quote-style-test
  (is (= "`toucan`"
...
(deftest debug-count-calls-test
  (testing "Test debug-count-calls"
    (is (= "DB Calls: 2\n"
  ...
  (testing "Test that identifiers are correctly lower cased in Turkish locale (toucan#59)"
    (let [original-locale (Locale/getDefault)]
      (try
        ...
        (query/execute! "DROP TABLE IF EXISTS heroes")
        (query/execute! "CREATE TABLE heroes (\"ID\" SERIAL PRIMARY KEY, \"NAME\" VARCHAR(256))")
        (query/execute! "INSERT INTO heroes (\"NAME\") VALUES ('Batman')")
